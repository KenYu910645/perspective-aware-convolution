...
anchors = edict(
        {
            ...
            'pyramid_levels':[3,  4,  5], # [i for i in range(3, 8)],   # [3,  4,  5,   6,   7]
            'strides': [8,  16, 32], # [2 ** (i) for i in range(3, 8)],  # [8,  16, 32,  64,  128]
            'sizes' : [32, 64, 128], # [4 * 2 ** i for i in range(3, 8)], # [32, 64, 128, 256, 512] # base_size
            'ratios': np.array([0.5, 1, 2.0]),
            'scales': np.array([2 ** (i / 3.0) for i in range(3)]), # [1, 1.26, 1,587]
            'is_pyrimid_external_anchor': True,
            'external_pixelwise_only_2d_anchor' : "/home/lab530/KenYu/ml_toolkit/anchor_generation/kmeans18_large_only_pyrimid_3_level_anchor/",
        }
    )

head_layer = edict(
    num_features_in=1024, # 256, 1024
    num_anchors= 6,# 8, # 32
...
detector.head = edict(
    ...
    data_cfg        = data,
    is_two_stage    = False,
    is_seperate_fpn_level_head = True, 
)
